...
class GH_cmd{
	...
	public  GH_cmd(String[] args) {
		...
		//	System.out.print("Enter the number of operation you would like to perform: ");
            ...
            switch (option) {
            	...
            	case 1: GH_DH(args);break; 
            	case 2: Sign (args);break;
                case 3: Verify(args);break; // for testing purpose
                //case 4: break;
	...
	//private static void GH_DH(BigInteger p, BigInteger a, BigInteger b, BigInteger Q, BigInteger X) {
	private static void GH_DH(String[] args) {
		BigInteger p = new BigInteger(args[1]);
		BigInteger a = new BigInteger(args[2]);
		BigInteger b = new BigInteger(args[3]);
		BigInteger Q = new BigInteger(args[4]);
		...
		//System.out.println("GH-DH key agreement");
        ...
        while (cont == 1) {
        	if (pri_key){ // Select Private Key
            	...
            	while (!GCD.equals((new BigInteger("1")))) {
            		System.out.print("Error: X is not co-prime with Q"); 
            		cont = 0;
            		break;
			...
			else if (cont != 0){ // Shared Key Computation
				//d2 = new Date();
                //t2 = d2.getTime();
                //elapsed_time = t2-t1;
				...
				a = new BigInteger(args[6]);
				b = new BigInteger(args[7]);
				//System.out.print("sx = " + a);
            	//System.out.print("s-x = " + b);
				//d1 = new Date();
				//t1 = d1.getTime();
			}
			Seq_term(p, a, b, s, X);
			if (pri_key && cont == 1) { // Output Public Key Pair
				//System.out.println("Your public key pair is (" + s[1] + "," + s[4] + ")");
				System.out.print("(" + s[1] + "," + s[4] + ");");
				pri_key = false;
			}
			else if (cont != 0){ // Output Shared Key Pair
	...
	private static void Sign (String[] args){
		...
		BigInteger Q = new BigInteger(args[4]);
		BigInteger X = new BigInteger(args[5]);
		BigInteger H = new BigInteger(args[6]);
		BigInteger K = new BigInteger(args[7]);
		BigInteger GCD, t;
		...
		*/
		...
		System.out.println("Sign a message");
		...
		while (delta_r_flag) { // Choose x such that delta != 0
			...
			GCD = X.gcd(Q);
			while (!GCD.equals((new BigInteger("1")))) {
            	System.out.println("Error: X is not co-prime with Q");
            	System.out.println("SIGNING IS NOT COMPLETED");
            	err_flag = true;
            	break;
            	//System.out.print("X is not co-prime with Q.  Select another x, 0 < x < " + Q + ": ");
            	//X = new BigInteger(ReadInput());
		...
		while (rt_flag) { // Choose k such that both r and t != 0 and delta != 0
			...
			while (delta_r_flag){ // Choose k such that delta != 0 and gcd(r, Q) = 1
				...
				GCD = K.gcd (Q);
				while (!GCD.equals((new BigInteger("1")))) {
            		System.out.println("Error: k is not co-prime with Q");
            		System.out.println("SIGNING IS NOT COMPLETED");
            		err_flag = true;
            		break;
	...
	private static void Verify(String[] args) {
		...
		BigInteger h = new BigInteger(args[5]);
		BigInteger r = new BigInteger(args[6]);
		BigInteger t = new BigInteger(args[7]);
		
		BigInteger u, v, GCD, sx,s_x;
		...
		//System.out.println("Verify a signature");
		...
		if (GCD.equals(new BigInteger("1"))) { // Case 1
			...
			s[1] = new BigInteger(args[12]); // sk
			s[2] = new BigInteger(args[13]); // sk+1
			...
			s[4] = new BigInteger(args[14]); // s-k
			s[5] = new BigInteger(args[15]); // s-(k+1)
			...
			sx = new BigInteger(args[8]); // sx
      		s_x = new BigInteger(args[10]); // s-x
			...
			//System.out.println("\nEnter the received (sx, sx+1):");
      		...
      		//s[5] = new BigInteger(ReadInput());
      		s[1] = new BigInteger(args[8]); // sx
			s[2] = new BigInteger(args[9]); // sx+1
			s[4] = new BigInteger(args[10]); // s-x
			s[5] = new BigInteger(args[11]); // s-(x+1)
		...
		//if ((s[1].equals(sx)) && (s[4].equals(s_x))) System.out.println("Valid signature\n");
		//else System.out.println("Invalid signature!!\n");
		if ((s[1].equals(sx)) && (s[4].equals(s_x))) System.out.print("Valid;");
