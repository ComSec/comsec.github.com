...
class GH2{
	...
	public  GH2(String[] args) {
        ...
        BigInteger Q = new BigInteger("1643138524243019099121322635788037230300368850983");
      	*/
      	...
      	System.out.println("Welcome to Gong-Harn Public-key Cryptosystem!");
      	int option;
        boolean cont = true;
	...
	private static void GH_DH(BigInteger p, BigInteger a, BigInteger b, BigInteger Q) {
		...
		while (cont == 1) {
        	if (pri_key){ // Select Private Key
            	...
            	X = new BigInteger("103929005321308650608990281194157653061304342136"); // hardcode for time testing
                ...
                //t2 = d2.getTime();
                //elapsed_time = t2-t1;
                //System.out.println("First part takes " + elapsed_time + "ms");
				//System.out.println("Enter the received key pair:");
				//System.out.print("sx = ");
				//a = new BigInteger(ReadInput());
            	//System.out.print("s-x = ");
				//b = new BigInteger(ReadInput());
				a = new BigInteger("2496178984246424089697443544768054765433214769679598465413285434505231766942510655554189012476724226207"); // hardcode for time testing
				b = new BigInteger("1237977654797716080592322547095488699585499214125287316717945124794956938535423980222582855213701634149"); // hardcode for time testing
			...
			if (pri_key) { // Output Public Key Pair
                ...
                //System.out.println("Second part takes " + (t2-t1) + "ms");
                elapsed_time = t2 - t1;
	...
	private static void sign (final BigInteger p, final BigInteger a, final BigInteger b, final BigInteger Q){
		...
		*/
		...
		H = new BigInteger("6495562832581790663061892574634853316331521383"); // hardcode for time testing
		...
		while (rt_flag) { // Choose k such that both r and t != 0 and delta != 0
			...
			while (delta_r_flag){ // Choose k such that delta != 0 and gcd(r, Q) = 1
				...
				K = new BigInteger("3831366776562807587262104399601647560268959215"); // hardcode for time testing
	...
	//private static void InvM0(final BigInteger p, final BigInteger a, final BigInteger b) { // for testing purpose
		...
		//System.out.println("This is the invert M0 result:");
	...
	private static void sk_plus_v(final BigInteger p, BigInteger a, BigInteger b, final BigInteger v, BigInteger[] s){
		...
		Seq_term(p, a, b, sv, v);
		//System.out.println("sv-1 state:");
        //for (int z = 0; z<6;z++) System.out.println(s[z]);
	...
	private static void Mixed_term(final BigInteger p, BigInteger a, BigInteger b, final BigInteger u, final BigInteger v, BigInteger[] s){
		//System.out.println("Inside Mixed Term 1: " + s[1] + "\n" + s[4]);
		if (!v.equals(new BigInteger("0"))) sk_plus_v(p, a, b, v, s); // to compute sk+v and its dual
		//System.out.println("Inside Mixed Term 2: " + s[1] + "\n" + s[4]);
	...
	private static void Verify(final BigInteger p, BigInteger a, BigInteger b, final BigInteger Q) {
		...
		h = new BigInteger("6495562832581790663061892574634853316331521383"); // hardcode for time testing
   		...
   		r = new BigInteger("2496178984246424089697443544768054765433214769679598465413285434505231766942510655554189012476724226207"); // hardcode for time testing
   		t = new BigInteger("489110567458434236826098070152095546688841265335"); // hardcode for time testing
		...
		if (GCD.equals(new BigInteger("1"))) { // Case 1
			...
			//System.out.println("\nEnter the received (sk, sk+1):");
      		...
      		s[1] = new BigInteger("2496178984246424089697443544768054765433214769679598465413285434505231766942510655554189012476724226207"); // hardcode for time testing
			s[2] = new BigInteger("1544101405933984423923639636184860947768866981998354810202122725005003896187444916202499552758690766103"); // hardcode for time testing
			s[4] = new BigInteger("1237977654797716080592322547095488699585499214125287316717945124794956938535423980222582855213701634149"); // hardcode for time testing
			s[5] = new BigInteger("74771894736472083733564775769110335710897742076494122940147360368994329808667560680908363933091781330"); // hardcode for time testing			
			...
			//System.out.println("\nEnter the received (sx, s-x):");
      		...
      		sx = new BigInteger("183390062233846550038690113460481547603153290669332396745442827514968810822014297911602857093318598675"); // hardcode for time testing			
			s_x = new BigInteger("1665633414385766744077700943938959538260734418541927107903010101748812024946616904305432267943538051790"); // hardcode for time testing			
		}
